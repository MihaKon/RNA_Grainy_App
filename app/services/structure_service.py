...
class StructureProcessor:
    ...
    def build_comparison_context(
        ...
        original_format = "mmcif" if file_format == SupportedFormats.CIF else file_format.value
        initial_data = {
            "job_id": job_id,
            "reference_url": f"/api/jobs/{job_id}/reference?file_format={file_format.value}", 
            "coarse_url": f"/api/jobs/{job_id}/coarse?file_format={COARSE_FILE_FORMAT.value}",
            "filename": filename,
            "file_format": [original_format, COARSE_FILE_FORMAT.value],
