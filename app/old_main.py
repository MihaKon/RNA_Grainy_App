...
from app.rcsb import RCSBNotFoundError, RCSBServiceError, fetch_rcsb_file
...
async def upload_file(
    ...
    if file_format not in SupportedFormats:
        return render_error_response(
            request, f"File format '{file_format}' is not supported.", 415
        )
...
async def fetch_rcsb(
    request: Request, rcsb_id: str | None = Form(None), selected_model: str = Form(...)
) -> HTMLResponse:
    if not rcsb_id:
